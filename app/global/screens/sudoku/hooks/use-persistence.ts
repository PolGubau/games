...
*/
  ...
  const getRanking = useCallback(async (): Promise<Sudoku[]> => {
    const ranking = await db.sudoku.where("win").equals(1).sortBy("completionTime");
  ...
  const createSudoku = async (board: Board, difficulty: number): Promise<Sudoku> => {
  ...
  const getStats = async () => {
    ...
    const allFinished = await db.sudoku.where("win").equals(1).or("lost").equals(1).toArray();
