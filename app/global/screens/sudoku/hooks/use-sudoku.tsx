...
export const useSudoku = () => {
	...
	const sudoku = useLiveQuery(() => db.sudoku.where("id").equals(id).first(), [id]);

	const lockedAmount = sudoku?.board?.flat().filter((cell) => cell.isLocked).length || 0;
	...
	const generateBoardCells: GenerateBoardCells = async (difficulty) => {
		...
		const boardWithHoles: Cell[] = board.map((cell: number) => {
			...
			const scale = Math.floor(
				(MAX_SHOWED_CELLS - MIN_SHOWED_CELLS) * difficultyPercent + MIN_SHOWED_CELLS,
			);
		...
		const rows: Row[] = boardWithHoles.reduce((rows: Row[], cell: Cell, index: number) => {
...
export const calculateTimeDiff = (timeStarted: Date, timeFinished: Date) => {
	...
	const string = `${hours ? `${hours}h ` : ""}${minutes ? `${minutes}m ` : ""}${seconds}s`;
